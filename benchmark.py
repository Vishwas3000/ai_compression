...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--jpeg-ai-encoder",
        help="Encoder command template using {input}, {output}, and {point}",
    )
    parser.add_argument(
        "--jpeg-ai-decoder", help="Decoder command template using {input} and {output}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for index, source in enumerate(sources, 1):
            reference_path = source_dir / f"{index:05d}-{source.stem}.png"
            ...
            for point in args.jpeg_ai_points:
                rows.append(
                    benchmark_jpeg_ai(
                        source, source_hash, reference_path, reference, point, args
                    )
                )
